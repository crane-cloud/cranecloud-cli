...
@click.option('-c', '--command', type=str, default='')
@click.option('-r', '--replicas', type=int, default=1)
@click.option('-o', '--port', type=int, default=80)
def deploy_app(project_id, name, image, command, replicas, port):
    ...
    try:
        ...
        response = requests.post(
            f"{API_BASE_URL}projects/{project_id}/apps", headers={'Authorization': f"Bearer {token}"}, data=data)
        response.raise_for_status()
        if response.status_code == 200:
            click.echo("App deployed successfully.")
        else:
            click.echo("Failed to deploy app.")
    except requests.RequestException as e:
        click.echo(e)
        ...
        if e.response or e.response.status_code == 404:
            ...
            click.echo(f"Failed to deploy app: {e.response.reason}")
        else:
            click.echo(f"Failed to connect to the server: {e}")
            click.echo(
                "Please check your internet connection or try again later."
